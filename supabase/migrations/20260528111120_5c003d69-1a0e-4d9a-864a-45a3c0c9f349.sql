-- Update get_dashboard_stats with security check
...
-- Update listar_membros_org with security check
CREATE OR REPLACE FUNCTION public.listar_membros_org(_org_id uuid)
 RETURNS TABLE(user_id uuid, email text, papel text, created_at timestamp with time zone)
 LANGUAGE plpgsql
...
DECLARE
    _caller_org_id uuid;
BEGIN
  -- SECURITY CHECK: Allow super_admin or organization gestor
  _caller_org_id := get_user_org(auth.uid());
  
  IF NOT (
    is_super_admin(auth.uid()) OR 
    (_caller_org_id = _org_id AND has_role(auth.uid(), 'gestor'::app_role))
  ) THEN
